#ifndef _INCLUDE_UNORDERED_SET_H_
#define _INCLUDE_UNORDERED_SET_H_
...
//
// 异常保证：
// yastl::unordered_set<Key> / yastl::unordered_multiset<Key> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
// 参数一代表键值类型，参数二代表哈希函数，缺省使用 yastl::hash，
// 参数三代表键值比较方式，缺省使用 yastl::equal_to
template <class Key, class Hash = yastl::hash<Key>, class KeyEqual = yastl::equal_to<Key>>
class unordered_set {
private:
  // 使用 hashtable 作为底层机制
...
public:
  ...
  unordered_set(unordered_set&& rhs) noexcept : ht_(yastl::move(rhs.ht_)) {}
...
public:
  ...
  unordered_multiset(unordered_multiset&& rhs) noexcept : ht_(yastl::move(rhs.ht_)) {}
  ...
  iterator emplace(Args&& ...args) {
    return ht_.emplace_multi(yastl::forward<Args>(args)...);
  }

  template <class ...Args>
  iterator emplace_hint(const_iterator hint, Args&& ...args) {
    return ht_.emplace_multi_use_hint(hint, yastl::forward<Args>(args)...);
  }
  ...
  iterator insert(value_type&& value) {
    return ht_.emplace_multi(yastl::move(value));
  }
  ...
  iterator insert(const_iterator hint, value_type&& value) {
    return ht_.emplace_multi_use_hint(hint, yastl::move(value));
  }
...
#endif // _INCLUDE_UNORDERED_SET_H_
