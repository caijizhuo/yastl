...
// 模板类 queue
// 参数一代表数据类型，参数二代表底层容器类型，缺省使用 yastl::deque 作为底层容器
template <class T, class Container = yastl::deque<T>>
class queue {
public:
...
public:
  ...
  queue(Container&& c) noexcept(std::is_nothrow_move_constructible<Container>::value) : c_(yastl::move(c)) {}

  queue(const queue& rhs) : c_(rhs.c_) {}

  queue(queue&& rhs) noexcept(std::is_nothrow_move_constructible<Container>::value) : c_(yastl::move(rhs.c_)) {}
  ...
  void emplace(Args&& ...args) {
    c_.emplace_back(yastl::forward<Args>(args)...);
  }
  ...
  void push(value_type&& value) {
    c_.emplace_back(yastl::move(value));
  }
  ...
  void swap(queue& rhs) noexcept(noexcept(yastl::swap(c_, rhs.c_))) {
    yastl::swap(c_, rhs.c_);
  }
...
// 模板类 priority_queue
// 参数一代表数据类型，参数二代表容器类型，缺省使用 yastl::vector 作为底层容器
// 参数三代表比较权值的方式，缺省使用 yastl::less 作为比较方式
template <class T, class Container = yastl::vector<T>, class Compare = yastl::less<typename Container::value_type>>
...
private:
  ...
  value_compare  comp_;  // 权值比较的标准 比较函数
...
public:
  ...
  explicit priority_queue(size_type n) : c_(n) {
    yastl::make_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  priority_queue(Container&& s) : c_(yastl::move(s)) {
    yastl::make_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  priority_queue(priority_queue&& rhs) : c_(yastl::move(rhs.c_)), comp_(rhs.comp_) {
    yastl::make_heap(c_.begin(), c_.end(), comp_);
  }
...
public:
  ...
  void emplace(Args&& ...args) {
    c_.emplace_back(yastl::forward<Args>(args)...);
    yastl::push_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  void push(value_type&& value) {
    c_.push_back(yastl::move(value));
    yastl::push_heap(c_.begin(), c_.end(), comp_);
  }

  void pop() {
    yastl::pop_heap(c_.begin(), c_.end(), comp_);
    c_.pop_back();
  }
  ...
  void swap(priority_queue& rhs) noexcept(noexcept(yastl::swap(c_, rhs.c_)) &&
                                          noexcept(yastl::swap(comp_, rhs.comp_))) {
    yastl::swap(c_, rhs.c_);
    yastl::swap(comp_, rhs.comp_);
  }
