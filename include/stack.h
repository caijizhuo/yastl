...
public:
  ...
  // 用container初始化
  stack(Container&& c) noexcept(std::is_nothrow_move_constructible<Container>::value) : c_(yastl::move(c)) {}

  stack(const stack& rhs) : c_(rhs.c_) {}

  stack(stack&& rhs) noexcept(std::is_nothrow_move_constructible<Container>::value) : c_(yastl::move(rhs.c_)) {}
  ...
  void swap(stack& rhs) noexcept(noexcept(yastl::swap(c_, rhs.c_))) {
