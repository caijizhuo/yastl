#ifndef _INCLUDE_ALGOBASE_H_
#define _INCLUDE_ALGOBASE_H_

// 这个头文件包含了 yastl 的基本算法
...
bool lexicographical_compare(const unsigned char* first1, const unsigned char* last1,
  ...
  // 先比较相同长度的部分
  const auto result = std::memcmp(first1, first2, yastl::min(len1, len2));
  // 若相等，长度较长的比较大
...
mismatch(InputIter1 first1, InputIter1 last1, InputIter2 first2)
{
  ...
  return yastl::pair<InputIter1, InputIter2>(first1, first2);
}
