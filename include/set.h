...
//
// 异常保证：
// yastl::set<Key> / yastl::multiset<Key> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
// 模板类 set，键值不允许重复
// 参数一代表键值类型，参数二代表键值比较方式，缺省使用 yastl::less 
template <class Key, class Compare = yastl::less<Key>>
class set {
public:
...
public:
  ...
  set(set&& rhs) noexcept : tree_(yastl::move(rhs.tree_)) {}
  // 拷贝赋值
...
public:
  ...
  multiset(multiset&& rhs) noexcept : tree_(yastl::move(rhs.tree_)) {}
  ...
  multiset& operator=(std::initializer_list<value_type> ilist) {
    tree_.clear();
    tree_.insert_multi(ilist.begin(), ilist.end()); // 允许重复值，这就是和 set 的区别
    return *this;
  }
